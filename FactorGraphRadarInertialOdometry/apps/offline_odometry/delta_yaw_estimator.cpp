...
int main() {
    ...
    std::string output_path = base_dir + "reve/cascade_delta_yaw_est.txt";
    ...
    Eigen::Quaterniond start_q(icp_config.start_qw, icp_config.start_qx, icp_config.start_qy, icp_config.start_qz);
    T_global_body.block<3,3>(0,0) = start_q.toRotationMatrix();
    ...
    std::cout << "Delta Yaw saved to: " << output_path << std::endl;
    return 0;
}