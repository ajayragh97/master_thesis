...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 1; i < synced_frames.size(); ++i)
    {
        ...
        auto rw_noise = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 
            cfg.graph.accel_bias_rw_sigma * std::sqrt(dt_frame), cfg.graph.accel_bias_rw_sigma * std::sqrt(dt_frame), cfg.graph.accel_bias_rw_sigma * std::sqrt(dt_frame),
            cfg.graph.gyro_bias_rw_sigma * std::sqrt(dt_frame), cfg.graph.gyro_bias_rw_sigma * std::sqrt(dt_frame), cfg.graph.gyro_bias_rw_sigma * std::sqrt(dt_frame)
        ).finished());
